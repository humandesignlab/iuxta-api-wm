...
const scrapeParams = require('./scraper-data');
...
const scraper = async (searchTerm) => {
	let searchPhrase = searchTerm;
	const browser = await puppeteer.launch({headless: false});
	
	scrapeParams.map( async (site, index) => {	
		if (site.unencodedURI) {
			searchPhrase = searchTerm.replace(/\s+/g, '+'); 
		} else {
			searchPhrase = encodeURI(searchTerm);
		}
		const page = await browser.newPage();
